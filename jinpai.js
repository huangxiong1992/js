...
async function category(tid, pg, _, ext = {}) {
    ...
    return JSON.stringify({ list: vodList, page: +pg, pagecount: 9999, limit: 90, total: 999999 });
}
async function detail(id) {
    const res = await reqSafe(`${currentHost}/api/mw-movie/anonymous/video/detail?id=${id}`, { headers: getHeaders({ id }) });
    const vod = normalizeVodList([res.data])[0];
    if (!vod) {
        return JSON.stringify({ list: [{ vod_id: id, vod_name: '加载失败', vod_play_url: '' }] });
    }
    vod.vod_play_from = '永久免费';
    if (vod.episodelist?.length) {
        // 修复分集全部显示1：循环取每一集自身name，兜底下标生成集数
        vod.vod_play_url = vod.episodelist.map((ep, index) => {
            const epName = ep.name && ep.name.trim() !== '' ? ep.name : `第${index + 1}集`;
            return `${epName}$${id}@@${ep.nid}`;
        }).join('#');
